...
void EvaluateAtmosphericScattering(PositionInputs posInput, float3 V, out float3 color, out float3 opacity)
{
    ...
    switch (fogType)
    {
        case FOGTYPE_LINEAR:
        {
            ...
            fogOpacity = _FogDensity * saturate((fragDist - _LinearFogStart) * _LinearFogOneOverRange) * saturate((_LinearFogHeightEnd - GetAbsolutePositionWS(posInput.positionWS).y) * _LinearFogHeightOneOverRange);
            fogColor  *= fogOpacity;
            break;
        }
        case FOGTYPE_EXPONENTIAL:
        {
            ...
            fogColor   = GetFogColor(V, fragDist);
            fogOpacity = _FogDensity * TransmittanceHomogeneousMedium(_ExpFogHeightAttenuation, fogHeight) * (1.0f - TransmittanceHomogeneousMedium(1.0f / _ExpFogDistance, fragDist));
            fogColor  *= fogOpacity;
            break;
        }
    ...
    if (atmosphereType == ATMOSPHERETYPE_PBR)
    {
        ...
        {
            // No geometry or atmosphere along the ray.
            return;
        }
        ...
        if (fogType == FOGTYPE_NONE)
        {
            ...
            color   = skyColor; // Pre-multiplied by design
            opacity = skyOpacity;
        }
        else
        {
